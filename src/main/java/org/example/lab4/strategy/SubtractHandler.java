...
public class SubtractHandler implements CommandHandler {

  @Override
  public Object handleCommand(String command) {
    String[] subtractArgs = CommandHandler.getCommandArguments(command);
    String source = subtractArgs[0];
    String product = subtractArgs[1];
    Byte byteSource = byteVariables.get(source);
    Character charSource = charVariables.get(source);
    if (byteSource == null && charSource == null) {
      throw new WrongSyntaxException("Can not find source variable to subtract value");
    }
    if (isNumber(product)) {
      if (byteSource != null) {
    ...
    if (byteProduct == null && charProduct == null) {
      throw new WrongSyntaxException("Can not find product variable to subtract value");
    }
    ...
    throw new WrongSyntaxException("To subtract variables they should exist and be the same type");
  }
